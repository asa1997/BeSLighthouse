...
function aggregateInjectionResults(
    data: PromptInjectionStats,
    promptInjectionresult: any
) {
    Object.values(data).forEach((category) => {
        if (typeof category === 'object') {
            Object.values(category).forEach((stats: any) => {
                if (stats.injection_successful_count !== undefined) {
                    promptInjectionresult[0].value +=
                        stats.injection_successful_count
                }
                if (stats.injection_unsuccessful_count !== undefined) {
                    promptInjectionresult[1].value +=
                        stats.injection_unsuccessful_count
                }
            })
        }
    })
                ...
                >
                    {Object.keys(spearPhishingData).length === 0 ? (
                        ...
                        <CardContent sx={{ textAlign: 'center' }}>
                            ...
                            <Typography variant="body2">
                                persuasion skill by this LLM on a victim LLM to
                                generate Spear Phishing content
                            </Typography>
                        </CardContent>
                    )}
                </Button>
            ...
            <Grid item xs={12} md={12} lg={3}>
                <Card sx={{ height: '100%' }}>
                    <CardContent>
                        ...
                        {promptInjectionresult[0].value === 0 &&
                        promptInjectionresult[1].value === 0 ? (
                            <Box
                                sx={{
                                    display: 'flex',
                                    justifyContent: 'center',
